; BMOS.asm - BeanZeeOS Machine Operating System (BeanDeck target)
;
; OS interface for BBC BASIC Z80 on BeanDeck hardware.
; Delegates I/O to Marvin via the jump table. BeanDeck extends
; BeanBoard with SPI TFT display and flash cartridge storage.
;
...
;
; File operations (OSSAVE, OSLOAD, etc.) are stubs. When Marvin
; gains SPI/flash jump table entries, these will be implemented
; to provide BASIC file I/O via flash cartridge storage.
;
...
;
; BeanDeck will implement these via Marvin's SPI/flash jump
; table entries when available. Until then, stubs.
;
